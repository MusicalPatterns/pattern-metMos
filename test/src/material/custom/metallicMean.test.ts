...
import { _1_MU_1, _2_MU_2, _3_MU_3, calculateMetallicMean, MET_MOS_PRECISION } from '../../../../src/indexForTest'

describe('metallic mean', () => {
    it('gives the correct value of the nth metallic mean', () => {
        expect(calculateMetallicMean(as.Ordinal(1)))
            .toBeCloseToTyped(_1_MU_1, MET_MOS_PRECISION)
        expect(calculateMetallicMean(as.Ordinal(2)))
            .toBeCloseToTyped(_2_MU_2, MET_MOS_PRECISION)
        expect(calculateMetallicMean(as.Ordinal(3)))
            .toBeCloseToTyped(_3_MU_3, MET_MOS_PRECISION)
        expect(calculateMetallicMean(as.Ordinal(4)))
            .toBeCloseToTyped(4.23606797749979, MET_MOS_PRECISION)
        expect(calculateMetallicMean(as.Ordinal(5)))
            .toBeCloseToTyped(5.19258240356725, MET_MOS_PRECISION)
        expect(calculateMetallicMean(as.Ordinal(6)))
            .toBeCloseToTyped(6.16227766016838, MET_MOS_PRECISION)
        expect(calculateMetallicMean(as.Ordinal(7)))
            .toBeCloseToTyped(7.14005494464026, MET_MOS_PRECISION)
        expect(calculateMetallicMean(as.Ordinal(8)))
